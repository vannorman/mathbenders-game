    ...
    function updateStyle() {
        if (currentInputFieldScript) {
            if (domInPlace && currentInputFieldScript.entity.element.screenCorners) {
                ...
                // Charlie: Have to modify this due to margins at top or left of my app.
                const leftPad = parseInt($('#application').css('margin-left')) * 1;
                const appHeight = parseInt($('#application').css('height'))
                const winHeight = window.innerHeight;
                const topMargin = parseInt($('#application').css('margin-top')); 
                const topPad = winHeight - appHeight - topMargin;
                inputDom.style.left = ((corners[0].x / devicePixelRatio) - 2) + leftPad + 'px';
                inputDom.style.bottom = ((corners[0].y / devicePixelRatio) - 2) + topPad + 'px';
