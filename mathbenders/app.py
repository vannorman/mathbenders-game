...
try:
    from mathbenders import db # Server import
except ImportError:
...
def serve_gzipped_js_playcanvas_engine():
    gzipped_path = os.getcwd()+'/mathbenders'+'/static/lib/playcanvas-stable.min.js.gz'
    @after_this_request
...
def serve_gzipped_js_ammo_wasm():
    gzipped_path = os.getcwd()+'/mathbenders'+'/static/lib/ammo/ammo.wasm.wasm.gz'
    @after_this_request
    def add_header(response):
        response.headers['Content-Encoding'] = 'gzip'
        response.headers['Content-Length'] = os.path.getsize(gzipped_path)
...
def serve_gzipped_js_ammo_wasm_js():
    gzipped_path = os.getcwd()+'/mathbenders'+'/static/lib/ammo/ammo.wasm.js.jgz'
    @after_this_request
    def add_header(response):
        response.headers['Content-Encoding'] = 'gzip'
        response.headers['Content-Length'] = os.path.getsize(gzipped_path)
