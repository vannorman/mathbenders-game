// Fix due to destruction/recreation of render texture multiple times causing an issue; todo is only ever have one using a static
// but this "fix" currently yields a black screen on the portal so something is wrong. todo investigate it.
...
PortalCameraRenderTexture.prototype.setupTexture = function(){
    RenderTextureService.Init();
    this.targetCam.camera.renderTarget = RenderTextureService.target;
...
class RenderTextureService {
    ...
    static Init(){
        if (!RenderTextureService.initialized){
            RenderTextureService.generate();
        }
    }
    ...
    static generate(){ 
        ...
        material.shader = shader; //Shaders.DefaultShader1();
        material.setParameter('uEmissiveMap', texture)


        RenderTextureService._material = material;
        RenderTextureService._texture = texture;
        RenderTextureService._target = renderTarget;
    }

    static get material(){
        if (!RenderTextureService.initialized) RenderTextureService.Init()
        return RenderTextureService._material;
    }

    static get texture(){
        if (!RenderTextureService.initialized) RenderTextureService.Init()
        return RenderTextureService._texture;
    }

    static get target(){
        if (!RenderTextureService.initialized) RenderTextureService.Init()
        return RenderTextureService._target;
    }
...
PortalCameraRenderTexture.prototype.postUpdate = function(dt){
    ...
    if (this.checkPortalTimer < 0 && pc.app.root.getComponentsInChildren('portal').length > 0){
        ...
        if (distToNearestPortal < distToCurrentPortal){
            ...
            this.renderPlane.render.material.emissiveMap = RenderTextureService.texture;     // assign the rendered texture as an emissive texture
            this.renderPlane.render.material = RenderTextureService.material; 
